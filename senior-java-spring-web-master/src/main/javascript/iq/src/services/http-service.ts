...
export class NoGenerateSignatureError extends CustomError {
	constructor() {
		super('Generate Signature Successfully');
	}
}

export class FileSizeTooLargError extends CustomError {
	constructor() {
		super('File size is too larg');
	}
}
	...
	private async handleResponse(response: Response): Promise<any> {
		...
		if (response.status < 200 || response.status >= 400) {
			if (response.status === 404) {
				...
				throw new NoGenerateSignatureError();
			}
			else if (response.status === 406) {
				throw new FileSizeTooLargError();
			}
